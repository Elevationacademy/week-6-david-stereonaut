...
app.get('/200to500', function (req, res) {
...
app.get('/lessThen5', function (req, res) {
  Book.find( {rating: { $lt: 5 } } ).sort( { author: 1 } ).then(function (result) {
    res.send(result)
  })
})
...
app.get('/fiction', function (req, res) {
  Book.find( { genres: "Fiction" }).skip(2).limit(3).then(function (result) {
    res.send(result)
  })
})
...
app.get('/tallAndRich', function (req, res) {
  Person.find( { $and: [ { height: { $gte: 180 } }, { salary: { $gte: 30000} } ] }, function (err, result) {
    res.send(result)
  })
})
...
app.get('/tallOrRich', function (req, res) {
  Person.find( { $or: [ { height: { $gte: 180 } }, { salary: { $gte: 30000} } ] }, function (err, result) {
    res.send(result)
  })
})
...
app.get('/skinnyGrey', function (req, res) {
  Person.find( { $and: [ { $or: [ { eyes: "grey"}, { hair: "grey" } ] }, { weight: { $lt: 70 } } ] }, function (err, result) {
    res.send(result)
  })
})
...
app.get('/greykids', function (req, res) {
  Person.find( { "kids.hair": "grey"}, function (err, result) {
    res.send(result)
  })
})
...
app.get('/overweight', function (req, res) {
  Person.find( { $and: [ { "kids.weight": { $gt: 100 } }, { weight: { $gt: 100 } } ] }, function (err, result) {
    res.send(result)
  })
})
